...
- Do not hash the live SAM when Windows has it locked.
- Use forensic acquisition for protected Windows artifacts.
- Do not assume `vssadmin` supports every VSS operation.
- Validate VSS paths before using them.
- Hash the acquired evidence copy.
- `Out-File` saves output but does not display it.
- Record missing Security events as telemetry gaps.
- Do not confuse general Wazuh visibility with confirmation of a specific event.
- Account names alone do not establish malicious activity.
- Use baseline comparison to identify account changes.
- Collect evidence before performing cleanup.
